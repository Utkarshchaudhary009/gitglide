...
import { Send, Play, Pause, Square, AlertCircle, CheckCircle2, Bot, User } from "lucide-react"

export default function SessionDetailPage() {
    const params = useParams()
    const id = params?.id as string
    const { sessions, activities, fetchActivities, sendMessage, refreshSession, approvePlan } = useSessionStore()
        ...
        <div className="flex h-[calc(100vh-theme(spacing.20))] flex-col gap-4">
            ...
            <div className="flex items-center justify-between border-b pb-4">
                <div className="space-y-1">
                    <h2 className="text-xl font-semibold tracking-tight">{session.title || "Untitled Session"}</h2>
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <Badge variant="outline" className="text-xs">{session.state}</Badge>
                        <span>•</span>
            ...
            {/* Chat / Activity Area */}
            <ScrollArea className="flex-1 rounded-md border p-4 bg-muted/20" ref={scrollRef}>
                <div className="flex flex-col gap-6">
                    ...
                    {sessionActivities.map((activity) => (
                        <div key={activity.id} className={`flex gap-3 ${activity.originator === 'user' ? 'flex-row-reverse' : ''}`}>
                            <Avatar className="h-8 w-8 mt-1">
                                {activity.originator === 'agent' ? (
                                    ...
                                    <div className="h-full w-full bg-zinc-600 flex items-center justify-center">
                            ...
                            <div className={`flex flex-col gap-1 max-w-[80%] ${activity.originator === 'user' ? 'items-end' : 'items-start'}`}>
                                <div className="flex items-center gap-2">
                                    <span className="text-xs font-semibold text-muted-foreground">
                                        {activity.originator === 'agent' ? 'GitGlide' : activity.originator === 'user' ? 'You' : 'System'}
                                    </span>
                                    <span className="text-[10px] text-muted-foreground/60">{new Date(activity.createTime).toLocaleTimeString()}</span>
                                </div>
                                <div className={`rounded-xl px-4 py-2.5 text-sm ${activity.originator === 'user'
                                ...
                                {activity.planGenerated && (
                                    <div className="mt-2 w-full rounded-md border bg-card p-4">
